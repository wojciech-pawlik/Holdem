...
public class RunController implements Initializable {
    ...
    private void drawCards() {
        ...
        for (Player player : board.getPlayers()) {
            ...
            var card1Img = new ImageView(new Image("file:" + (new File("").getAbsolutePath()) +
                    PATH_TO_CARDS + player.getCard(0).getName() + ".png"));
            ...
            var card2Img = new ImageView(new Image("file:" + (new File("").getAbsolutePath()) +
                    PATH_TO_CARDS + player.getCard(1).getName() + ".png"));
    ...
    private void drawFlop() {
        ...
        var flopPanes = new Pane[]{(Pane) boardPane.getChildren().get(0), (Pane) boardPane.getChildren().get(1), (Pane) boardPane.getChildren().get(2)};
        var flopImageViews = new ImageView[]{
                new ImageView(new Image("file:" + (new File("").getAbsolutePath() +
                        PATH_TO_CARDS + board.getCards().get(FLOP_CARD1).getName() + ".png"))),
                new ImageView(new Image("file:" + (new File("").getAbsolutePath() +
                        PATH_TO_CARDS + board.getCards().get(FLOP_CARD2).getName() + ".png"))),
                new ImageView(new Image("file:" + (new File("").getAbsolutePath() +
                        PATH_TO_CARDS + board.getCards().get(FLOP_CARD3).getName() + ".png")))
        };
        for(ImageView imageView : flopImageViews)
            configureImageView(imageView, CARD_WIDTH, CARD_HEIGHT, CARD_PANE_STYLE);
        for(int i = 0; i < flopPanes.length; i++)
            flopPanes[i].getChildren().add(flopImageViews[i]);
    }

    private void drawTurn() {
        board.drawCards(TURN);
        var turn = (Pane) boardPane.getChildren().get(3);
        var turnImg = new ImageView(new Image("file:" + (new File("").getAbsolutePath() +
                PATH_TO_CARDS + board.getCards().get(TURN_CARD).getName() + ".png")));
    ...
    private void drawRiver() {
        ...
        var river = (Pane) boardPane.getChildren().get(4);
        var riverImg = new ImageView(new Image("file:" + (new File("").getAbsolutePath() +
                PATH_TO_CARDS + board.getCards().get(RIVER_CARD).getName() + ".png")));
